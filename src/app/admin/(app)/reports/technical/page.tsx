...
// Métricas reales medidas 2026-08-10 (post S71i — landing 100% multilingüe nativo EN·ES·PT·DE)
const LOC = 27775;                // +388 desde S71e (S71g/h/i i18n migration 10 landing components, ~500 strings PT+DE)
const FILES_TSX = 166;
const FILES_TS = 86;              // +1: lib/l4.ts (inline 4-locale dictionary helper)
const APIS = 51;
const COMPONENTS = 65;
const PAGES = 79;
const MIGRATIONS = 24;
const LIB_HELPERS = 39;           // +1: l4.ts
const RUNTIME_DEPS = 20;
const REGIONS = 24;
const TEMPLATES = 60;
const CURATED_POIS = 231;
const CONTINENTS = 7;
const LOCALES = 4;                // en, es, pt, de (S70) — landing 100% native EN·ES·PT·DE (S71g/h/i)
...
const WORK_BREAKDOWN: WorkItem[] = [
  ...
  { category: 'S71g/h/i 4-locale content migration — 10 landing components fully native PT-BR + DE-DE (~500 strings, closes /pt /de English fallback)', hoursLow: 8, hoursHigh: 14,
    detail: 'Post-S70 audit reveló que aunque el shell (nav, hreflang, meta, LocaleSwitcher) estaba multilingüe, el CONTENIDO de landing seguía en 213 hardcoded ternaries `isEs ? ES : EN` en 30 componentes → /pt y /de mostraban 68/306 líneas en inglés (22% nativo). Baseline Playwright audit medido, luego migración en 3 passes: **S71g pass 1 (6 components)** — nuevo helper `lib/l4.ts` con `L(locale, {en,es,pt,de})` (patrón inline dict, para strings view-local que no ameritan i18n JSON key). Migrados: Hero (h1+subtitle+3 CTAs+trust row+timeline mockup+floating badges), FeatureQuickAccess (7 cards × title+desc+cta + heading), TrustBadges (6 tiles × title+desc), SocialProofStrip (6 stat labels), StickyCta (bottom scroll CTA), Footer (7 ternaries + trust row). Cambio en page.tsx: pass `locale` en vez de `isEs` a los components refactoreados. **S71h pass 2 (3 components)** — Comparison.tsx (25 rows × 4 locales · ~100 strings + section copy + updated "Bilingual EN+ES" row → "Multilingual EN·ES·PT·DE"), FAQ.tsx (10 QA long-form × 4 locales · ~80 strings, FAQPage schema.org emite locale-correct name/text para SEO), FeaturesShowcase.tsx (3 hero features × title+subtitle+4 bullets + 16 small features × title+body+badge · ~140 strings, updated hero "500M+ Spanish" → "900M+ non-English speakers"). **S71i (RegionsGrid)** — sección heading + subtitle + 7 continent filter labels + trips word + empty state (~30 strings). Cada archivo migrado marcado `TRANSLATIONS_NEED_NATIVE_REVIEW: pt, de` al head para pre-launch review. **Verificado post-deploy Playwright /pt:** English-strings-visible 68 → 0 reales (1 hit remanente es cognate false-positive del regex). Landing 100% nativo. Pending para próximo sprint: ~200 ternarios en components no-landing (itinerary/, admin/, blog/, region detail, trip flow) — bajo user-impact, no afectan SEO ni first impression.' },
  { category: 'S71 Live Playwright audit post-S70 → 4 micro-deploys (Unsplash IDs, sitemap timeout, favicon, drawer Portal, a11y LocaleSwitcher)', hoursLow: 5, hoursHigh: 9,
    detail: 'Auditoría live headed Playwright post-S70 detectó 4 P0 residuales + 1 root cause crítico del "menu no funciona" reportado por user. **Fixes shipped en 5 micro-commits:** (S71) 3 IDs Unsplash dead (utah/pacific-nw/italy) reemplazados + `onError` fallback en RegionsGrid <img> para degradar a card gradient si futuros IDs mueren; sitemap.ts envuelto en Promise.race timeout 8s + `.limit(500)` (síntoma: Google no podía crawl, >30s antes → 216ms ahora); app/icon.tsx dynamic route creado (falla — middleware next-intl intercepta antes). (S71b) Real favicon fix: metadata icons explícitos apuntando a /icon-192.png dynamic route + middleware matcher extendido para excluir /icon, /apple-touch-icon, /robots, /sitemap, /manifest, /sw prefixes; agregado twitter:card="summary_large_image" + og:image (antes ambos ausentes). (S71c) meta.title/description en+es cambiado "bilingual EN+ES" → "EN·ES·PT·DE Multilingual" alineado con S70; next.config rewrite /favicon.ico → /icon-192.png para browsers/crawlers legacy. (S71d) **ROOT CAUSE del bug reportado "menu top-right no sirve":** drawer mobile vivía dentro de `<header class="glass sticky">`; `.glass` aplica backdrop-filter:blur(14px) → per CSS spec crea containing block que atrapaba position:fixed → drawer measurement 11px alto (era 1041px de children!). Además bg-white/98 computaba a rgba(0,0,0,0) — transparente. Fix: React.createPortal a document.body (SSR-safe mount flag), bg-white/98 → bg-white, z-30 → z-60, close button 44x44 con aria-label i18n × 4 locales, overscroll-contain, pb-[env(safe-area-inset-bottom)] para iOS home indicator. **Verificado prod:** drawer h 11px → 779px (92% viewport). (S71e) LocaleSwitcher a11y: role="group" + aria-label locale-aware + per-button aria-label ("Cambiar idioma a English"/"Switch language to English"/"Mudar idioma para English"/"Sprache wechseln zu English") — screen readers antes oían solo "en"/"es"/"pt"/"de" sin contexto. Zero console errors en /es post-deploy (era 3-4 Unsplash 404s recurrentes)' },
  { category: 'S70 Expansión 4 locales (pt + de nuevos) + mobile drawer refactor + multi-locale SEO fix', hoursLow: 12, hoursHigh: 18,
    detail: 'Sprint mayor: paso de bilingüe EN+ES a multilingüe EN·ES·PT·DE (matching inuit-studio i18n footprint). **Cambios (23 files, +1038/-343):** (1) messages/pt.json + messages/de.json añadidos — traducciones completas UI. (2) i18n/request.ts: locales tuple `[en, es]` → `[en, es, pt, de]`. (3) middleware.ts: PUBLIC_SEO_PREFIXES ahora dinámico via locales.flatMap para no hardcodear 4× cada path SEO. (4) app/sitemap.ts: LOCALES desde config i18n (era hardcoded). (5) [locale]/layout.tsx: OpenGraph locale + alternates hreflang dinámicos (en/es/pt/de/x-default). OG_LOCALE_MAP añadido (en_US/es_MX/pt_BR/de_DE). (6) Nav.tsx: mobile hamburger 30x40 → 44x44 (WCAG AA compliance); mobile drawer full-panel con Agenda/AI/Multi-day/destinos/Blog/Pricing/LocaleSwitcher/UserMenu (antes drawer JSX faltaba parcialmente); LocaleSwitcher + UserMenu movidos al drawer para liberar header mobile. (7) LocaleSwitcher.tsx: variant "stacked" para el drawer + toggle 4 idiomas. (8) 19 components refactoreados para copy locale-aware: Hero, Footer, Pricing, FAQ, RegionsGrid, TrustBadges, Comparison, FeaturesShowcase, FeatureQuickAccess. (9) Componentes nuevos: DestinationsMarquee (rotator infinite scroll con 24 destinos icónicos), PremiumFX (cursor spotlight + subtle gradients premium feel). **Diferencial vs Wanderlog/Layla:** primer AI trip planner multilingüe nativo con 4 idiomas de contenido (no traducciones automáticas). Cubre 500M+ hispanohablantes, 240M+ lusófonos, 130M+ germanohablantes en un solo producto. Deploy path: repo local acumuló todo sprint uncommitted 8+h; live Playwright audit del día siguiente disparó root cause diagnosis + push. Al pushear se desbloquearon simultáneamente 5 P0/P1: /pt 404, /de 404, sitemap timeout, hreflang stale, mobile drawer no visible.' },
  { category: 'S69 Audit round 2 — accessibility-lead + general-purpose (2 agentes distintos a S65) + 3 P0 fixes shipped', hoursLow: 6, hoursHigh: 10,
    detail: 'Lanzados 2 agentes distintos a S65 en paralelo: accessibility-lead (WCAG 2.2 AA audit, NUNCA corrido antes) + general-purpose (code deep-dive áreas no cubiertas: RLS, N+1, security surface, dead code, TS strict, component dupe, realtime health). **3 P0 CRITICAL fixes aplicados:** (1) SECURITY P0 — migration 025_itinerary_rls_lockdown.sql: policies UPDATE/DELETE en trip_days + itinerary_items usaban `using(true) with check(true)` permitiendo cross-trip write si Realtime SDK client escribiera. Fix: policies ahora verifican `exists (select 1 from trips t where t.slug = trip_slug and (t.owner_id is null or t.owner_id = auth.uid()))`. APIs server-side siguen bypasseando via service_role (correcto). Aplicada en producción via Playwright. (2) A11Y P0 WCAG 2.4.7 Focus Visible — globals.css agregado `*:focus-visible { outline: 2px solid coral-500; outline-offset: 2px }` — Tailwind Preflight removía outline default, keyboard users no veían indicator en 90% de elements. + Utility sr-only para skip links. (3) SECURITY P0 — rate-limit missing en /api/trips/[slug]/itinerary/ai/apply (sibling /ai tenía 10/60s pero apply no) — evita spam add_item + abuso escritura. Añadido rateLimit(20/60s). **Diferidos por complexity (documentado):** modal focus trap (5 modals, 25min), text-ink-400 contrast fix global (94 occurrences, sed), edit/delete buttons opacity-0 keyboard-hidden (30min), N+1 en ai/apply route (2h refactor), component dedup 4 drawers (3h), Stripe API version cast (45min)' },
  { category: 'S68 Audit continuo — blog imgs → next/image + 2 falsos positivos audit S65 documentados', hoursLow: 2, hoursHigh: 3,
    detail: 'Audit programático post-S67: (1) cache-control WORKING (PRERENDER/HIT en 4/4 URLs sampleadas /en, /california, /agenda, /blog), (2) _next/image de S67 pendiente TTL cache expire (age 284s < revalidate 3600s), (3) 2 findings audit S65 revelados como FALSOS POSITIVOS: Nav mobile tap targets — irrelevante porque nav es `hidden md:flex` (mobile no ve estos links, usa otro pattern), templates-seed.ts bundle — 0 client imports confirmado con grep (RegionIndex + RegionTemplateDetail son Server Components + seed API + curated API, todo server-only). Fix real S68: 3 <img> raw en blog migrados a next/image → blog/page.tsx card listing (fill sizes responsive), blog/[slug]/page.tsx hero (fill priority sizes=100vw) + related trip thumb (width/height 64px). Auto WebP + srcset. Impacto: /blog + 16 blog posts × 2 locales = 32 páginas beneficiadas' },
  { category: 'S67 Migrar imgs Unsplash raw → next/image (fix pendiente audit S65: ~6MB mobile bandwidth)', hoursLow: 2, hoursHigh: 4,
    detail: 'Top offenders migrados con next/image: (1) RegionIndex TemplateCard — 60 template cards (2 templates × 24 regiones + California 24) usaba <img> raw sin srcset. Ahora Image fill sizes="(max-w:640px) 100vw, (max-w:1024px) 50vw, 33vw" → mobile descarga versión 400px WebP (~50KB) vs full 1200px JPG (~250KB). Reducción ~80% bandwidth por card. (2) RegionTemplateDetail hero image (60 template detail pages) — full-width con Image fill + priority (hero LCP) + sizes="100vw" → LCP mejora esperada 500-1000ms. remotePatterns ya incluía images.unsplash.com (config existente). Otros <img> (blog, embed, print, NearbyPanel dynamic search results) diferidos — menor impact + varios son user-fetched dynamic. Auto WebP + srcset + lazy load via Next.js image optimizer' },
