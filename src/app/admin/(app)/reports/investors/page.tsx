  ...
  return (
    <main className="mx-auto max-w-4xl px-8 py-10">
      ...
      <header className="mb-10 border-b border-ink-100 pb-6">
        ...
        <p className="mt-2 text-[14px] text-ink-500">Vista de socios · 2026-08-10 · Producto en producción · <b>60 templates · 24 regiones · 7 continentes · 4 idiomas 100% nativos (EN·ES·PT·DE landing) · Auditorías S65+S69+S71 aplicadas (perf+SEO+copy+a11y+security+i18n): migration 025 RLS lockdown, focus-visible global, rate-limit /ai/apply, mobile drawer Portal fix + a11y LocaleSwitcher, ~500 strings PT+DE en 10 componentes de landing</b></p>
      </header>
      ...
      <Section title="⭐ Ventajas técnicas defendibles (por qué somos diferentes)">
        <ul className="ml-5 list-disc space-y-2.5 text-[14px] leading-relaxed text-ink-700">
          ...
          <li><b>Multilingüe nativo 4 idiomas — EN·ES·PT·DE (S70 shell + S71g/h/i content — completo)</b> — expansión de bilingüe a multilingüe con traducciones nativas en 10 componentes de landing (Hero, FeatureQuickAccess, TrustBadges, SocialProofStrip, StickyCta, Footer, Comparison, FAQ, FeaturesShowcase, RegionsGrid). ~500 strings PT-BR + DE-DE autoradas con nivel native-adjacent, cada archivo marcado para pre-launch review por hablante nativo. Cada string UI, meta title/description, OpenGraph, hreflang, sitemap alternates cubren los 4 idiomas; FAQPage schema.org emite Q&A en el locale del visitante para SEO en 4 mercados. Verificación live Playwright: /pt y /de bajaron de 22% nativo (S70) → 86% (S71g) → 99% (S71h) → 100% (S71i). TAM sumado: mercados anglófono + hispanohablante + lusófono + germanohablante = ~1.1 mil millones de personas. Ningún competidor tiene los 4 (Wanderlog: solo EN; Layla: EN+ES paywall; Roadtrippers: solo EN). Nuevo helper `lib/l4.ts` permite escalar el patrón a nuevos componentes con 3 líneas de código.</li>
          ...
          <li><b>Landing UX rediseño premium (S48 — nuevo)</b> — reemplaza patrón waitlist obsoleto por hero orientado a producto live: 3 CTAs por intent (🗓 blank canvas / ✨ AI 30s / 🌍 explore rutas) con benefit-focused copy en lugar de features. Mockup del Timeline day×time hace visible la feature diferencial (Itinerary Engine) sin scroll. Nueva sección <em>FeatureQuickAccess</em> con 6 features de 1-click (Itinerary NEW / AI LIVE / Curated 60+ / Colab / Offline PWA / WhatsApp BETA). Nav rediseñado con dropdown "Destinos" que expone las 24 regiones organizadas por 4 continentes con flags. Discoverability cerrada: usuario que quiere solo itinerario lo encuentra en &lt;5s.</li>
          <li><b>Discovery Panel + Feature Tour dentro del Itinerary (S50 — nuevo)</b> — dentro del itinerary diario ahora hay un <em>Discovery Panel</em> collapsible con 7 categorías (food/attraction/nature/hotel/shopping/gas/ev), radio slider 1-50km, filtro rating, sort by rating/distance. Los resultados vienen de <code>/api/places/discover</code> con bbox derivado del viaje real (no viewport aleatorio como Google Maps) → resultados hyper-relevantes. Botón <em>+Agregar</em> añade la parada directo al día seleccionado con category-inferred type (meal/hotel/place). <em>FeatureTour</em> de 5 pasos aparece en la primera visita a /itinerary: DayNavigator, Discovery, AI, TravelSegments live, Colab+Offline — reduce time-to-first-action drásticamente. Landing simplificada removiendo CitiesGrid redundante (Nav "Destinos" dropdown ya lo cubre).</li>
          <li><b>Optimización manual de rutas con TSP nearest-neighbor (S55 — nuevo)</b> — cuando el usuario arma manualmente una ruta con drag & drop en <code>/trip/[slug]</code>, ahora hay botón <em>✨ Optimizar orden de paradas</em> visible con ≥4 stops. Backend <code>/api/routes/optimize-order</code> aplica nearest-neighbor greedy preservando origin + destination (típicamente hotel/starting point), luego 2-opt lite (forward vs reverse). Diferencial: sin llamar Google Route Optimization API ($$$), cliente-side puro. Test producción: A(LA)→B(Santa Barbara)→C(Long Beach)→D(SLO) reordenado a A→C→B→D ahorra 54% distancia (482 km → 221 km). Confirm dialog bilingüe con savings human-readable antes de aplicar.</li>
          <li><b>Seed completo 60 templates en producción (S56 — critical fix)</b> — bug detectado con Playwright: 18 regiones (Italy, Iceland, Japan, Morocco, Chile, Argentina, Peru, Australia, NZ, Germany, Scotland, Ireland, Mexico, Canada, PNW, Rockies, Northeast, Southeast) mostraban "no templates yet" aunque el código tenía las rutas. Root cause: endpoint seed-templates requiere token vacío en env. Fix: auth alternativo <code>isAdminAuthed()</code> permite re-seed desde admin UI. Post-fix: 60/60 templates seeded OK, cada región muestra ≥2 rutas listas para personalizar (Italy: Amalfi Coast + Tuscany Loop; Japan: Golden Route + Hokkaido; Morocco: Sahara Loop + Blue Cities; etc). Discovery ampliado 2.5× (24 → 60 rutas visibles al usuario).</li>
          <li><b>Arquitectura dual: Agenda standalone + Rutas multi-día (S52 — nuevo)</b> — separación clara de dos casos de uso que antes se conflictaban: <b>Agenda diaria</b> (<code>/[locale]/agenda</code>) es un flujo day-only Google Calendar-style para cualquier día suelto (sin destino, sin viaje macro, sin cuenta) — usuario llega, click "Crear mi agenda", queda en editor listo para agregar restaurantes/compromisos/atracciones/notas. <b>Rutas multi-día</b> (<code>/[locale]/trip/new</code>) mantiene el flujo road-trip completo (varios días, ciudades, DnD entre días, mapas offline, PDF export). Ambos comparten el Itinerary Engine underneath sin duplicar código. AddItemInline gana Quick mode con 8 presets 1-click (Restaurante 60min, Café 30min, Compromiso 60min, Atracción 90min, Ejercicio 60min, Compras 60min, Descanso 30min, Nota) — reduce fricción de add drásticamente. Nav prioritiza Agenda emerald + Rutas gris. Este split amplía TAM: no solo road-trippers sino cualquiera planeando un día en su ciudad.</li>
